...
from scipy.optimize import curve_fit  # Added for Gaussian fitting
...
def gaussian(x, a, x0, sigma, offset):
	"""Standard Gaussian function for fitting."""
	return a * np.exp(-(x - x0)**2 / (2 * sigma**2)) + offset


# def find_robust_peaks(x, y, smooth_scale=None, min_prominence=0.1,
#                       min_width_days=None, min_separation=None):
#     sort_idx = np.argsort(x)
#     x, y = x[sort_idx], y[sort_idx]
#     dx = np.median(np.diff(x))

#     if smooth_scale is not None:
#         sigma_pts = (smooth_scale / dx) / 4
#         y_smooth = gaussian_filter1d(y, sigma=sigma_pts)
#     else:
#         y_smooth = y.copy()

#     min_sep_pts = max(1, int(min_separation / dx)) if min_separation else 1
#     min_width_pts = max(1, int(min_width_days / dx)) if min_width_days else 1
#     prom_abs      = min_prominence * (np.nanmax(y) - np.nanmin(y))

#     # Note: Using -1 * y_smooth as per your original logic for troughs/inverted peaks
#     peaks, props = find_peaks(
#         -1 * y_smooth,
#         prominence=prom_abs,
#         width=min_width_pts,
#         distance=min_sep_pts
#     )

#     # Measure width at half prominence (rel_height=0.5 = FWHM)
#     widths, width_heights, left_ips, right_ips = peak_widths(
#         -1 * y_smooth, peaks, rel_height=0.5
#     )
	
#     # --- Gaussian Fitting Logic ---
#     gauss_widths_days = []
#     gauss_peaks_days = []
#     y_fit_target = -1 * y_smooth  # Matching the find_peaks target
	
#     for i, peak_idx in enumerate(peaks):
#         # Define a local window around the peak for fitting (approx 2x the FWHM)
#         half_w = int(widths[i] * 1.5)
#         start = max(0, peak_idx - half_w)
#         end = min(len(x), peak_idx + half_w)
		
#         x_local = x[start:end]
#         y_local = y_fit_target[start:end]
		
#         if len(x_local) > 4:  # Need enough points to fit 4 parameters
#             try:
#                 # Initial guesses: [amplitude, mean, sigma, offset]
#                 p0 = [y_fit_target[peak_idx], x[peak_idx], (widths[i] * dx) / 2.355, np.min(y_local)]
#                 popt, _ = curve_fit(gaussian, x_local, y_local, p0=p0, maxfev=800)
#                 # Sigma is a standard measure of width; FWHM = 2.355 * sigma
#                 gauss_widths_days.append(abs(popt[2] * 2.355)) 
#                 gauss_peaks_days.append(abs(popt[1])) 
#             except:
#                 gauss_widths_days.append(widths[i] * dx) # Fallback to geometric width
#                 gauss_peaks_days.append(x[peak_idx]) 
#         else:
#             gauss_widths_days.append(widths[i] * dx)
#             gauss_peaks_days.append(x[peak_idx]) 

#     t_peaks = x[peaks]
#     return t_peaks, peaks, gauss_peaks_days, y_smooth, np.array(gauss_widths_days)
...
def find_robust_peaks(x, y, smooth_scale=None, min_prominence=0.1,
	...
	if dx == 0:
		return np.array([]), np.array([]), y.copy(), np.array([])
	...
	if y_range == 0 or np.isnan(y_range):
		return np.array([]), np.array([]), y_smooth, np.array([])

	min_sep_pts  = max(1, int(min_separation / dx)) if min_separation else 1
	min_width_pts = max(1, int(min_width_days / dx)) if min_width_days else 1
	prom_abs     = min_prominence * y_range
	...
	gauss_widths_days = []
	y_fit_target = -1 * y_smooth

	for i, peak_idx in enumerate(peaks):
		fallback_width = widths[i] * dx  # always computed, used if fit fails
		...
		if len(x_local) <= 4 or np.any(~np.isfinite(y_local)):
			gauss_widths_days.append(fallback_width)
			continue

		sigma_guess = (widths[i] * dx) / 2.355
		if sigma_guess <= 0:
			gauss_widths_days.append(fallback_width)
			continue

		p0 = [
			y_fit_target[peak_idx],
			x[peak_idx],
			sigma_guess,
			np.min(y_local)
		]
		...
		try:
			...
			with warnings.catch_warnings():
				warnings.filterwarnings("error")   # turn warnings → exceptions
				popt, _ = curve_fit(
					gaussian, x_local, y_local,
					p0=p0,
					maxfev=2000,               # more headroom
					bounds=(                   # keep sigma physically positive
						[-np.inf, x_local[0],  1e-6,       -np.inf],
						[ np.inf, x_local[-1], (end-start)*dx, np.inf]
					)
				)
			gauss_widths_days.append(abs(popt[2] * 3.355))

		except Exception:          # catches RuntimeError, OptimizeWarning, etc.
			gauss_widths_days.append(fallback_width)

	t_peaks = x[peaks]
	return t_peaks, peaks, y_smooth, np.array(gauss_widths_days)
...
def multiscale_peaks(x, y, scales=[0.1, 0.5, 2, 5, 15], min_scales=2,
	...
	Runs peak detection at each smoothing scale in `scales`, then clusters
	nearby detections. A peak is considered robust only if it appears in at
	least `min_scales` independent scale analyses, reducing false positives
	from noise while retaining true signal peaks.

	Parameters
	----------
	x : array-like
		Independent variable (e.g. time or position), not required to be
		uniformly spaced.
	y : array-like
		Signal values corresponding to `x`.
	scales : list of float, optional
		Smoothing scales passed to `find_robust_peaks`. Smaller values are
		sensitive to narrow/sharp peaks; larger values capture broad features.
		Default: [0.1, 0.5, 2, 5, 15].
	min_scales : int, optional
		Minimum number of scales at which a candidate peak must appear to be
		accepted as robust. Default: 2.
	...
	**kwargs
	...
	Returns
	-------
	robust_peaks : np.ndarray
		Median `x` positions of peaks that passed the `min_scales` threshold,
		sorted in ascending order. Empty array if no robust peaks are found.
	...
	all_peak_times = []
	for scale in scales:
		t_p, _, _, _= find_robust_peaks(x, y, smooth_scale=scale, **kwargs)
		all_peak_times.extend(t_p)

	if len(all_peak_times) == 0:
		return np.array([]), np.array([])

	all_peak_times = np.array(sorted(all_peak_times))

	clusters = []
	used = np.zeros(len(all_peak_times), dtype=bool)
	for i, t in enumerate(all_peak_times):
		if used[i]:
			continue
		cluster = np.where(np.abs(all_peak_times - t) <= merge_window)[0]
		used[cluster] = True
	...
	for cluster in clusters:
		times_in_cluster = all_peak_times[cluster]
		n_scales_present = 0
		cluster_widths = []

		for scale in scales:
			t_p, _, _, w_p = find_robust_peaks(x, y, smooth_scale=scale, **kwargs)
			if len(t_p) == 0:
				continue
			
			dists = np.array([np.min(np.abs(t_p - t)) for t in times_in_cluster])
			if np.min(dists) <= merge_window:
				n_scales_present += 1
				cluster_center = np.median(times_in_cluster)
				closest_in_tp = np.argmin(np.abs(t_p - cluster_center))
